...
class RobustMongo:
    ...
    async def store_payload(self, collection_name: str, job_id: str,
                            payload: bytes,
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to update existing payload for job {job_id}: {e}")
                    if attempt == max_retries - 1:
                        raise MongoConnectionError(
                            f"Failed to store payload after {max_retries} attempts: {e}")
                    await asyncio.sleep(
                        0.1 * (2 ** attempt))  # Exponential backoff
            except PyMongoError as e:
                logger.warning(
                    f"Failed to store payload for job {job_id} (attempt {attempt + 1}): {e}")
                if attempt == max_retries - 1:
                    raise MongoConnectionError(
                        f"Failed to store payload after {max_retries} attempts: {e}")
                await asyncio.sleep(
    ...
    async def insert_payload_auto_id(self, collection_name: str,
                                     payload: Optional[bytes],
                                     metadata: Optional[Dict] = None) -> str:
        """Insert payload without specifying _id and return the generated
        ObjectId as a string. Retries on transient failures with exponential
        backoff.
        """
        collection = self.get_collection(collection_name)
        document = {
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(
                    f"Failed to insert payload (attempt {attempt + 1}): {e}")
                if attempt == max_retries - 1:
                    raise MongoConnectionError(
                        f"Failed to insert payload after {max_retries} attempts: {e}")
                await asyncio.sleep(0.1 * (2 ** attempt))

    async def retrieve_payload(self, collection_name: str, job_id: str) -> \
            Optional[bytes]:
        ...
        for attempt in range(max_retries):
            try:
                document = None
                # Try ObjectId form first if applicable, then fall back to
                # string id for backward compatibility
                ...
                logger.warning(
                    f"Failed to retrieve payload for job {job_id} (attempt {attempt + 1}): {e}")
                if attempt == max_retries - 1:
                    raise MongoConnectionError(
    ...
    async def delete_payload(self, collection_name: str, job_id: str) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(
                    f"Failed to delete payload for job {job_id} (attempt {attempt + 1}): {e}")
                if attempt == max_retries - 1:
                    raise MongoConnectionError(
    ...
    async def bulk_delete_payloads(self, collection_name: str,
        ...
        for attempt in range(max_retries):
            try:
                object_ids = [ObjectId(j) for j in job_ids if
                              isinstance(j, str) and ObjectId.is_valid(j)]
                string_ids = [j for j in job_ids if not (
                        isinstance(j, str) and ObjectId.is_valid(j))]
                ...
                if not filters:
                    return 0
                query: Dict[str, Any] = filters[0] if len(filters) == 1 else {
                    "$or": filters}
                result = await collection.delete_many(query)
                return result.deleted_count
            except PyMongoError as e:
                logger.warning(
                    f"Failed to bulk delete payloads (attempt {attempt + 1}): {e}")
                if attempt == max_retries - 1:
                    raise MongoConnectionError(
                        f"Failed to bulk delete payloads after {max_retries} attempts: {e}")
                await asyncio.sleep(
                    0.1 * (2 ** attempt))  # Exponential backoff
    ...
    async def clear_collection(self, collection_name: str) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(
                    f"Failed to clear collection '{collection_name}' (attempt {attempt + 1}): {e}")
                if attempt == max_retries - 1:
                    raise MongoConnectionError(
                        f"Failed to clear collection after {max_retries} attempts: {e}")
                await asyncio.sleep(
                    0.1 * (2 ** attempt))  # Exponential backoff
...
class MongoManager:
    ...
    async def initialize(cls, connection_string: str, database_name: str):
        ...
        if cls._client is None:
            try:
                ...
                logger.info(
                    f"MongoDB connected successfully to {database_name}")

            except Exception as e:
                logger.error(f"Failed to initialize MongoDB connection: {e}")
                raise MongoConnectionError(
                    f"Failed to initialize MongoDB: {e}")
    ...
    def get_client(cls) -> RobustMongo:
        ...
        if cls._robust_mongo is None:
            raise MongoConnectionError(
                "MongoDB not initialized. Call initialize() first.")
